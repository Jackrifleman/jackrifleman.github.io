...
attributes = [
        [
            ...
            "Mini-crits vs burning players", "Does not require ammo", "Projectiles penetrate players",
            "+~INT health gained per second ~ATYPE", "On Hit: Damage dealt is returned as ammo", "+~PER% max ammo on wearer",
            ...
            "On Hit Teammate: They gain ~STATUS for ~INT seconds", "On Hit: Target is ~STSNEG for ~INT seconds", "Extinguishes teammates",
            ...
            "+~PER% damage bonus vs minors", "+~PER% damage bonus vs buildings", "100% critical hits vs burning players", "100% critical hits from behind",
            "Mini-crits from behind", "Dealing ~MTRDMG damage fills the ~METER meter, alt-click to activate ~METER, and gain ~STATUS and ~STATUS for ~INT seconds"
        ],
        [
            "-~PER% damage penalty", "-~PER% clip size", "-~PER max health on wearer", "On Damage: Become ~STATUS for ~INT seconds",
            "On Damage: Instantly die while deployed", "On Miss: Hit yourself. Idiot.", "No random critical hits", "On Miss: Lose -~PER health",
            "On Miss: Instantly die", "~PER% slower move speed ~ATYPE", "-~PER% less accurate", "Damage decreases as the user becomes injured",
            "-~INT health drained per second ~ATYPE", "~PER% slower weapon switch speed ~ATYPE", "Fires tracer rounds", "-~PER% max ammo on wearer",
            ...
            "Accuracy decreases as the user becomes injured", "Mini-crits whenever it would normally crit", "-~PER% damage penalty vs buildings",
...
statuses = [
        [
            "100% critical chance", "a speed boost", "mini-crits", "invulnerability", "+~PER% damage resistance",
            "backstab immunity", "afterburn immunity", "headshot immunity", "+~PER% ~DMG damage resistance",
            "+~PER max health", "~DMG damage immunity", "afterburn immunity"
        ],
        [
            "slowed by ~PER%", "marked for death", "ignited", "doused in Mad Milk", "doused in Jarate",
            "feared", "stunned", "~PER% more vulnerable to damage", "~PER% more vulnerable to ~DMG damage"
        ]
]
...
function getAttribute(neg) {
    ...
    _ind = irandom(attributes[neg].length);
    while(_attribs[neg].includes(_ind)) {
        _ind = irandom(attributes[neg].length);
    }

    _attribs[neg].push(_ind);

    _att = attributes[neg][_ind];
    _percent = Math.floor((((irandom(99)))+5)/5) * 5;
    _int = Math.floor((((irandom(29)))+5)/5) * 5;
    _mtr = meters[irandom(meters.length)];
    _mtrDmg = 50 * (irandom(8) + 2);
    ...
    while(_att.search("~") != -1) {
        ...
        _att = _att.replace("~METER", _mtr);
        _att = _att.replace("~MTRDMG", _mtrDmg.toString());
